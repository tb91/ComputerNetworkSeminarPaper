...
\newcommand{\seminartitel}{Analyse des modified Yao-Steps angewandt auf den Delaunay Graphen}
...
Für jedes $k \geq 14 $ existiert ein subgraph $G' $ vom Delaunay-Graph $G $, sodass jeder Knoten aus $G' $ einen maximalen Ausgangsgrad von k und einen stretch-factor von $1+2\pi(k \cos{\frac{\pi}{k}})^{-1} $ hat.
...
Um den Pfad von A nach B zu erhalten, wird ein Pfad von A zum intermediate Point T und von diesem zu B erstellt und konkateniert.
...
\begin{figure}[h!]
\centering
\includegraphics[width=1\linewidth]{outward_path_empty.eps}
