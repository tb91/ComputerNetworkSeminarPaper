...
\item[Basisfall] Die Rekursion endet, wenn $AB \in G$. (siehe Abbildung \ref{fig:outward_path1})
\item[Rekursionsfall] Aufgrund der Delaunay-Eigenschaft, dass eine Kante nur dann nicht in $G $ enthalten ist, wenn im Kreis $\bigcirc {ABC} $
...
\caption{ $(O_1) $ und $(O_2) $ sind vollständig innerhalb von (O)}
...
\caption{ $\alpha $ und $\beta $ sind beide kleiner als $ AOB $}
